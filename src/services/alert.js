...
class AlertService extends EventEmitter {
  ...
  registerAlert(alert, market) {
    ...
    console.log(
      `[alert/${alert.user}] create alert ${market} @ ${alert.price} (user now have ${
        this.alertEndpoints[alert.endpoint].count
      } active alerts)`
    )
  ...
  unregisterAlert(alert, market, wasAutomatic) {
    ...
    if (index !== -1) {
      ...
      if (wasAutomatic) {
        console.log(`[alert/${user || 'unknown'}] server removed ${market} @ ${alert.price} (user now have ${count} active alerts)`)
      } else {
        console.log(`[alert/${user || 'unknown'}] user removed alert ${market} @ ${alert.price} (user now have ${count} active alerts)`)
      }
  ...
  moveAlert(alert, market, newPrice) {
    ...
    if (index !== -1) {
      ...
      console.log(`[alert/${alert.user}] move alert on ${market} @ ${alert.price} -> ${newPrice} (user now have ${count} active alerts)`)
  ...
  async scheduleAlertsCleanup(alert, market) {
    setTimeout(() => {
      ...
      if (this.alertEndpoints[alert.endpoint] && !this.alertEndpoints[alert.endpoint].count) {
        console.log(
          `[alert/cleanup] no more alerts for ${this.alertEndpoints[alert.endpoint].user}`
        )
